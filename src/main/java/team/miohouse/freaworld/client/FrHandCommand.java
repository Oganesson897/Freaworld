...
public class FrHandCommand implements ClientCommandRegistrationCallback, Command<QuiltClientCommandSource> {

	@Override
	public void registerCommands(CommandDispatcher<QuiltClientCommandSource> dispatcher) {
		dispatcher.register(ClientCommandManager.literal("frhand").executes(this));
	}
	...
	public int run(CommandContext<QuiltClientCommandSource> context) throws CommandSyntaxException {
		...
		MutableText heading = ((MutableText) stack.getName()).styled(style -> style.withColor(Formatting.GREEN)
				.withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, Text.of(itemId.toString())))
		...
		stack.getItem().getBuiltInRegistryHolder().streamTags()
				.forEach(tag -> tags.append(new LiteralText(" #" + tag.id().toString()).styled(style -> style
						.withColor(Formatting.YELLOW)
						.withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, Text.of(tag.id().toString())))
